...
class TestSystem(BaseSystemTest):
    ...
    async def test_endpoint_exists_and_returns_response(
        ...
        async with AsyncClient(
            app=test_app, base_url=self.url_prefix
        ) as client:
            res = await client.get(self.endpoint_url)
            assert isinstance(res, Response)

    async def test_endpoint_returns_401_if_token_is_invalid(
        self, test_app: FastAPI
    ):
    ...
    async def test_endpoint_returns_403_if_no_auth_token(
        self, test_app: FastAPI
    ):
        async with AsyncClient(
            app=test_app, base_url=self.url_prefix
    ...
    async def test_endpoint_returns_exact_dict_data(
        self, test_app: FastAPI
    ):
        async with AsyncClient(
            app=test_app, base_url=self.url_prefix
    ...
    async def test_endpoint_returns_correct_status(
        self, test_app: FastAPI
    ):
        async with AsyncClient(
            app=test_app, base_url=self.url_prefix
